...
{
    string version = appConfig.GetSection("Version").Value;
    IConfigurationSection Logging = appConfig.GetSection("AppConfig");
    string LevelLogging = Logging["LevelLogging"];
    string TypeLogger = Logging["TypeLogging"];
    ...
    FacultyRepository facultyRepository =
        new FacultyRepository(logger, universityRepository, workerRepositoryAdministrator);
    DepartmentRepository departmentRepository =
        new DepartmentRepository(logger, facultyRepository, workerRepositoryAdministrator);
    ...
    Thread threadOfInfo = new Thread(() =>
    {
        ...
        {
            Console.WriteLine(
                "Вывод интересующей вас инфо. Если о рабочих, введите 1, если о студентах, введите 2. Если о баллах студентов - 3, если о пропусках студентов - 4");
